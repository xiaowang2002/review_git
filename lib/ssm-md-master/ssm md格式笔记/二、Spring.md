...
这是 IOC 容器的基本实现，是 Spring 内部使用的**接口**。面向 Spring 本身，不提供给开发人员使用。

#### ②ApplicationContext

BeanFactory 的子**接口**，提供了更多高级特性。面向 Spring 的使用者，几乎所有场合都使用
...
注意：**建议使用ClassPathXmlApplicationContext，他具有跨平台的应用，而FileSystemXmlApplicationContext只能在当前电脑使用**
...
xml用spring config方式创建，名字随便去，不一定要取applicationContext.xml

***注意：接口不能作为bean组件***

```xml
<!--
        xmlns：命名空间，解决了xml元素重名的问题
        xmlns:xsi：使用xsi作为前缀的命名空间
        xsi:schemaLoation：约束的位置
-->
<!--
	配置HelloWorld所对应的bean，即将HelloWorld的对象交给Spring的IOC容器管理
	通过bean标签配置IOC容器所管理的bean
	属性：
		id：设置bean的唯一标识[可以不写，但不能有多个同样类的bean，并且不能通过id获取]
		class：设置bean所对应类型的全类名
-->
<bean id="helloworld" class="com.atguigu.spring.bean.HelloWorld"></bean>  <!--默认调用无参构造器，如果没有，到时候运行该bean的时候会报错-->
```
...
public void testHelloWorld(){
    ...
    HelloWorld helloworld = (HelloWorld) ac.getBean("helloworld");  //默认调用无参构造器，如果没有无参构造器，则报错
    helloworld.sayHello();
}
```
...
***在没有进行依赖注入之前，Spring 底层默认通过反射技术调用组件类的==无参构造器==来创建组件对象，这一点需要注意。如果在需要无参构造器时，没有无参构造器，则会抛出下面的异常：***
...
bean的id唯一性，类型可以重复（但不能用类型获取bean，因为spring也不知道要取那个bean,或者用id和类型获取或者只用id获取）
...
***除非你获取bean时，通过id或者id和class（确立唯一性），而不止是class***
...
**即通过bean的类型，bean所继承的类的类型（父类），bean所实现的接口的类型都可以获取bean**
...
==property属性是通过set方法赋值的，所以组件类中必须有set方法==

```xml
<bean id="studentOne" class="com.atguigu.spring.bean.Student">
    <!-- property标签：通过组件类的setXxx()方法给组件对象设置属性，所以组件类中必须有set方法 -->
...
> ```
>
> **以上写法，为name所赋的值是字符串null，并不是为真正意义上的null**
...
<property name="expression">
    ...
    <!-- 所以CDATA节中写什么符号都随意 -->
    <value><![CDATA[a < b]]></value> <!--在标签外面可以不用加" "-->
</property>
<!--CDATA节单独放在一个value标签里-->
```
...
#### ④方式二：内部bean（类似于局部内部类，仅供内部，外部获取不到）
...
<bean id="studentFour" class="com.atguigu.spring.bean.Student">
    ...
    <property name="clazz" ref="clazzOne"></property>    <!--引入，使当前bean有clazz类-->
    <property name="clazz.clazzId" value="3333"></property> <!--算的上修改吧-->
    <property name="clazz.clazzName" value="最强王者班"></property> <!--算的上修改吧-->
</bean>
```
...
<bean id="studentFour" class="com.atguigu.spring.bean.Student">
    ...
    <property name="hobbies">
        <array>
            <value>抽烟</value>  <!--value标签的值写在外面，并且不用加" "-->
            <value>喝酒</value>
            <value>烫头</value>
        </array>
    </property>
...
<bean id="clazzTwo" class="com.atguigu.spring.bean.Clazz">
    ...
    <property name="students">
        <list>
            <ref bean="studentOne"></ref>  <!--bean是指某个组件的id-->
...
<property name="subjects">
            <set>
                <value>语文</value>   <!--引用类型用ref，基本数据类型用value-->
                <value>数学</value>
                <value>英语</value>
            </set>
</property>
...
<bean id="studentFour" class="com.atguigu.spring.bean.Student">
    ...
    <property name="teacherMap" ref="teacherMap"></property>   <!--你也可以用第②种的内部赋值-->
</bean>
```
...
引入p命名空间后，可以通过以下方式为bean的各个属性赋值，**p 命名空间是 setter 方式属性注入的一种快捷实现方式**
...
引用resource下的文件记得加classpath:，如果要引用的文件在resource的一个包下，记得包名也要加上
...
public void testBeanScope(){
    ...
    System.out.println(user1==user2);  //sinleton为true，prototype为false
}
```
...
- bean对象创建（调用无参构造器）
- 给bean对象设置属性（依赖注入）
- bean对象初始化之前操作（由bean的后置处理器负责，自动调用）
- bean对象初始化（需在配置bean时指定初始化方法，自动调用）
- bean对象初始化之后操作（由bean的后置处理器负责，自动调用）
- bean对象就绪可以使用
- bean对象销毁（需在配置bean时指定销毁方法，ioc容器关闭前时自动调用）
- IOC容器关闭
...
public void testLife(){
    ...
    ac.close();  //先自动调用销毁方法在关闭，注意 ApplicationContext没有close()，所有要用classPathXmlApplicationContext
}
```
...
且配置到IOC容器中，需要注意的是，***bean后置处理器不是单独针对某一个bean生效，而是针对IOC容器(一个spring文件)中所有bean都会执行***
...
> <!-- bean的后置处理器要放入IOC容器才能生效，并且可以不用写id -->
>
...
> 自动装配方式：***byType***
>
> byType：根据类型匹配IOC容器中的某个兼容类型（当前**属性类**，或者是**当前属性类的子类**或**当前属性类的实现类**）的bean，为**属性**自动赋值
>
> 若在IOC中，没有任何一个兼容类型的bean能够为属性赋值，则该属性不装配，即值为默认值
>
> null
>
> 若在IOC中，有**多个兼容类型的bean能够为属性赋值，则抛出异常**（==所以不能有多个兼容的==）
>
...
> 自动装配方式：***byName***
>
> byName：将自动装配的属性的**属性名**，作为**bean的id**在IOC容器中匹配相对应的bean进行赋值，找不到就报错
...
举例：元 旦联欢会要布置教室，蓝色的地方贴上元旦快乐四个字，红色的地方贴上拉花，黄色的地方贴上气球。
...
<context:component-scan base-package="com.atguigu" use-default-filters="false">
    ...
    <!--
     ...
     -->
    <!--
         context:include-filter:指定扫描的类，使用前，先把use-default-filters设置为false,
         否则无效
    -->   
...
**在成员变量上直接标记@Autowired注解即可完成自动装配**，不需要写成员变量的getter、setter方法；因为我们是从ioc容器中注入的。以后我们在项目中的正式用法就是这样。
...
> **@Autowired注解可以标记在构造器（有参构造）和set方法（因为这两个都可以给要用的成员变量赋值）**上**和标记在属性上结果一样**
>
> 带有@Autowired注解（属性required默认为true）的**构造方法**，在Spring初始化对象实例时，会调用此构造函数，进行对象的实例化。
...
- 首先根据所需要的组件类型到IOC容器中查找
  ...
  - 和所需类型匹配的bean不止一个
    ...
    - 能够找到：执行装配
    - 找不到：装配失败
    - 使用@Qualifier注解：**根据@Qualifier注解中==指定==的名称作为bean的id进行匹配**（就算使用了@Qualifier注解，还是先用bytype装配）
    - 能够找到：执行装配
    - 找不到：装配失败
...
```java
public class C	alculatorPureImpl implements Calculator {
    @Override
...
解决这两个问题，核心就是：**解耦。我们需要把附加功能从业务功能代码中*抽取*出来**。

#### ③困难

解决问题的困难：**要抽取的代码在方法内部，靠以前把连续的子类中的重复代码抽取到父类的方式没法解决。但现在不是连续的，是分散的，所以需要引入新的技术。**
...
public class ProxyFactory {
    private Object target;    //因为是通用的代理类，所以不知道目标类是什么类型
    public ProxyFactory(Object target) {    //通过构造器获得目标类类型
        this.target = target;
    }
    public Object getProxy(){
         ...
         * 其中有三个参数：
         * 1、classLoader：加载动态生成的代理类的类加载器,我们编写的.java文件需要先编写成.class文件，如果需要用到某个类，JVM通过类加载器把对应的.class文件加载到内存中，然后再执行代码
         * 2、interfaces：目标对象实现的所有接口的class对象所组成的数组
         * 3、invocationHandler：设置代理对象实现目标对象方法的过程，即代理类中如何重写接口中的抽象方法
         */
        ClassLoader classLoader = target.getClass().getClassLoader();
        Class<?>[] interfaces = target.getClass().getInterfaces();
        InvocationHandler invocationHandler = new InvocationHandler() {   //也可以用lambda表达式
            @Override
            public Object invoke(Object proxy, Method method, Object[] args) //参数1无所谓，参数2是目标类的目标方法，参数3是目标方法的参数
                throws Throwable {
                /**
                 * proxy：代理对象
...
public void testDynamicProxy(){
    ProxyFactory factory = new ProxyFactory(new CalculatorLogImpl()); //这里的CalculatorLogImpl方法里只有核心业务逻辑，没有多余的附加功能（像日志）
    Calculator proxy = (Calculator) factory.getProxy();   //用被目标类实现的接口接收，因为目标类和最后的动态代理后的类是并列的
    proxy.div(1,0);
    //proxy.div(1,1);
}
```
...
**AOP（Aspect Oriented Programming）是一种设计*思想*，是软件设计领域中的面向切面编程**，它是面向对象编程的一种补充和完善，它以通过预编译方式和运行期动态代理方式实现在不修改源代码的情况下给程序动态统一添加额外功能的一种技术。

### 3.3.2、相关术语

#### ①横切关注点（抽取出来的非核心业务，是一些，不是某个）
...
**每一个横切关注点上**要做的事情都需要写一个方法来实现，这样的方法就叫通知方法。

- 前置通知：在被代理的目标方法**前**执行
- 返回通知：在被代理的目标方法**成功结束**后执行（**寿终正寝**）
- 异常通知：在被代理的目标方法**异常结束**后执行（**死于非命**）
- 后置通知：在被代理的目标方法**最终结束**后执行（**盖棺定论**）
- 环绕通知：使用**try...catch...finally结构围绕整个被代理的目标方法**，包括上面四种通知对应的所

有位置

**后置通知和返回通知的区别是，后置通知:不管方法是否有异常，都会执行该通知；而返回通知是方法正常结束时才会执行。**
...
向目标对象应用**通知之后**创建的**代理对象**。

#### ⑥连接点

这也是一个纯逻辑概念，不是语法定义的。

把方法排成一排，每一个横切位置看成x轴方向，把方法从上到下执行的顺序看成y轴，x轴和y轴的交叉点就是连接点。连接点是一个虚拟的概念，可以理解为所有满足切点扫描条件的所有的时机

![23](img\23.png)

#### ⑦切入点

**定位连接点的方式。切入点一定是连接点，反正不一定。我们要使用的那个连接点就是切入点，（特指现在要用到的连接点）**
...
为这个技术要求**代理对象和目标对象实现同样的接口**（兄弟两个拜把子模式），**就像我上面写的那个动态代理。**

- cglib：通过**继承被代理的目标类**（认干爹模式）实现代理，所以不需要目标类实现接口。

- AspectJ：本质上是静态代理，**将代理逻辑“织入（把切面（aspect）连接到其它的应用程序类型或者对象上，并创建一个被通知（advised）的对象，类似于插入，融入的意思）”被代理的目标类编译得到的字节码文件**，所以最终效果是动态的。weaver就是织入器。Spring只是借用了AspectJ中的注解。
...
public class LoggerAspect {
    ...
    第一个*号表示任意的访问修饰符和返回值类型
    第二个*表示类中的任意方法
    ..表示任意的参数列表
    类的地方可以使用*，表示包下所有的类
    包的地方也可以使用*，表示任意的包
    */
//    @Before("execution(public int com.atwzs.spring.aop.annotation.Calculator.add(int ,int ))") //精准
    @Before(value = "execution(* com.atwzs.spring.aop.annotation.CalculatorImpl.*(..))")  //CalculatorImpl下的任意方法
    public void beforeAdviceMethod(JoinPoint joinPoint) {  //jointPoint封装了连接点
        //获取连接点的签名信息（方法名）
        String methodName = joinPoint.getSignature().getName();
        //获取目标方法到的实参信息
        String args = Arrays.toString(joinPoint.getArgs());   //jointpoin.getArgs()获取实参
        System.out.println("Logger-->前置通知，方法名：" + methodName + "，参数：" + args);
    }

    @Pointcut("execution(* com.atwzs.spring.aop.annotation.CalculatorImpl.*(..))")    //切入点表达式
    public void pointCut() {    //方法名可以乱取，但是后面调用的方法名要和这个一致
    }
...
![image-20230423213434739](C:\Users\王志生\AppData\Roaming\Typora\typora-user-images\image-20230423213434739.png)
...
@AfterReturning中的属性returning，用来将通知方法的某个形参，接收目标方法的返回值，如果目标方法返回类型为void，则result为null,相当于没有返回，用的result的默认值
...
### 3.4.8、环绕通知（环绕通知和那四个普通通知二选一即可）

环绕通知是有返回值的,目标方法为void类型返回null（即result为null）。
...
- @Order(较大的数)：优先级低
  ...
  **注意：@Order注解在切面上（通知类上）**,@Order(1),切面的优先级,默认值Integer的最大值
...
<aop:config>
    <!--配置切面类-->
    <aop:aspect ref="loggerAspect">        <!--ref指的是存储通知方法的bean-->
        <aop:pointcut id="pointCut" expression="execution(*com.atguigu.aop.xml.CalculatorImpl.*(..))"/>   <!--切入点也可以写在外面，aop:config里面-->
        <aop:before method="beforeMethod" pointcut="com.atwzs.spring.annotation.LogAspect.test()"></aop:before>
...
<!--注意：调用方法的pointcut用pointcut,引用配置的用pointcut-ref-->
```
...
当我们想引用resource包下的某个文件时，我们要加上classpath：,如果那个文件在某个包下，包名也要添上

```java
@RunWith(SpringJUnit4ClassRunner.class)  //@RunWith(SpringJUnit4ClassRunner.class),让测试运行于Spring测试环境
@ContextConfiguration("classpath:spring-jdbc.xml")  //当我们想要在某个测试类使用@Autowired注解来引入这些收集起来的bean时，只需要给这个测试类添加@ContextConfiguration注解来标注我们想要导入这个测试类的某些bean。
...
query()可以查出来多个，而queryObject()只能查出来一个，看个人需求
...
//        User user = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(User.class), 35);
        List<User> list = jdbcTemplate.query(sql, new BeanPropertyRowMapper<>(User.class), 35);
//        System.out.println(user);
...
使用`BeanPropertyRowMapper`将数据库查询结果转换为`Java`类对象。如果你不是自定义类对象，直接类型.class，像如下就可
...
==orm依赖有jdbcTemplate，transaction等lib==

spring-test是spring的test要用的，像runWith注解就要用到这个依赖，通常配合junit一起使用
...
购买图书之后，用户的余额为-30，数据库中余额字段设置了==无符号==，因此无法将-30插入到余额字段
...
<!--transaction-manager的值和事务配置的id一致-->
```
...
因为service层表示业务逻辑层，一个方法表示一个完成的功能，因此处理事务一般在==service层==处理
...
```java
@Transactional(readOnly = true)   //只允许读，出现写就报错
public void buyBook(Integer bookId, Integer userId) {
    //查询图书的价格
    Integer price = bookDao.getPriceByBookId(bookId);
    //更新图书的库存
...
- rollbackForClassName属性：需要设置一个==字符串类型的全类名==
...
```java
@Transactional(noRollbackFor = ArithmeticException.class) //异常还是会报，但不会回滚了
...
**可以通过@Transactional中的==propagation属性==设置事务传播行为**
...
<tx:advice id="txAdvice" transaction-manager="transactionManager">
    <tx:attributes>
        <!--如果要给某个方法加事务，一定要通过tx:method显示设置出来，否则则没有事务的功能，不能依靠切入点，它只是给个定位，并不配置事务-->
        <!-- tx:method标签：配置具体的事务方法 -->
        <!-- name属性：指定方法名，可以使用星号代表多个字符 -->
        <tx:method name="get*" read-only="true"/>
        <tx:method name="query*" read-only="true"/>
        <tx:method name="find*" read-only="true"/>
        <tx:method name="*"/>       <!--连接点中所有的方法都配置-->
        <!-- read-only属性：设置只读属性 -->
