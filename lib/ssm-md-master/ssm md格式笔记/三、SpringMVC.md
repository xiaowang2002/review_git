...
## 4.2、准备工作(控制层交给springmvc组件管理，service层及其他层交给spring组件管理)
...
```xml
<!--扫描组件，springmvc只需要扫描控制层，其他组件（包括Service层等）交给spring管理-->
...
有些配置原本在mybatis-config，现在转移到spring配置文件上
...
<beans xmlns="http://www.springframework.org/schema/beans"
    ...
    <!--扫描组件，不需要扫描controller注解上的bean，因为归mvc管-->
    ...
    <!-- 配置用于创建SqlSessionFactory的工厂bean,Mybatis-spring包提供，可以直接在spring的ioc中获取sqlSessionFactory -->
    <bean class="org.mybatis.spring.SqlSessionFactoryBean">
        ...
        <!--
            设置映射文件的路径
            若映射文件所在路径和mapper接口所在路径一致，则不需要设置,mybatis的mappers配置也可以省略
        -->
        <!--
